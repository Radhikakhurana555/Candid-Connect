    ...
    if not requisitions:
        ...
        with st.form("candidate_form"):
            ...
            with col2:
                ...
                total_experience = st.number_input("Total experience", min_value=0.0, max_value=50.0, step=0.5)
                source = st.selectbox("Source", ["LinkedIn", "Referral", "Job Portal", "Vendor", "Internal Database", "Other"])
                verification_status = st.selectbox("Verification status", VERIFICATION_STATUSES, index=2)
        ...
        if submitted:
            if not full_name.strip():
                ...
                app_id = create_application(candidate_id, req_map[selected_req]["requisition_id"])
...
with assess_tab:
    ...
    if not applications:
        ...
        app_map = {f"{a['full_name']} | {a['job_title']} | {a['application_id']}": a for a in applications}
        ...
        st.write(f"**Current role:** {application.get('current_title') or 'Not available'} at {application.get('current_company') or 'Not available'}")
        ...
        if refreshed.get("score_bucket"):
            ...
            st.dataframe(pd.DataFrame(details)[["criterion_name", "weight", "awarded_score", "evidence"]], use_container_width=True, hide_index=True)
