...
with create_tab:
    ...
    with st.form("requisition_form", clear_on_submit=False):
        ...
        with col1:
            ...
            experience_min = st.number_input(
                "Minimum experience", min_value=0.0, max_value=50.0, value=5.0
            )
        with col2:
            ...
            experience_max = st.number_input(
                "Maximum experience", min_value=0.0, max_value=50.0, value=12.0
            )
        ...
        criteria_df = st.data_editor(
            ...
            column_config={
                "criterion_type": st.column_config.SelectboxColumn(
                    options=["keyword", "experience", "location"]
                ),
    ...
    if submitted:
        ...
        if not job_title.strip():
        ...
        elif st.session_state.get("last_requisition_submission") == fingerprint:
            st.warning("This requisition submission has already been processed.")
        else:
            duplicate = find_duplicate_requisition(
                job_title=job_title,
            ...
            if duplicate:
                st.warning(
                    "A matching open requisition already exists: "
                    f"{duplicate['requisition_id']} | {duplicate['job_title']}."
                )
            else:
                st.session_state["last_requisition_submission"] = fingerprint
